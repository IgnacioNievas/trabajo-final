...
void kruskal(Grafo **lista){//recibo una lista ordenada
    ...
    Arista *resultado = (Arista*)malloc((VERTICES) * sizeof(Arista));//genero un puntero que almacenara las aristas del arbol reducido
    assert(resultado != NULL);
    Subconjunto *sub = (Subconjunto*)malloc(VERTICES * sizeof(Subconjunto));//genero un puntero de subconjuntos
    assert(sub != NULL);
